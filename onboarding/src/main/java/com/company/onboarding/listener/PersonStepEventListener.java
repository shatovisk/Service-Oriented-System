...
@Component
public class PersonStepEventListener {

    @Autowired
    private DataManager dataManager;

    @EventListener
    public void onUserStepChangedBeforeCommit(EntityChangedEvent<PersonStep> event) {
        User user;
        if (event.getType() != EntityChangedEvent.Type.DELETED) {
            Id<PersonStep> userStepId = event.getEntityId();
            PersonStep personStep = dataManager.load(userStepId).one();
            user = personStep.getUser();
        } else {
